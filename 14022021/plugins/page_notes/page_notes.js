...
(function () {
    ...
    $(window.document).ready(function () {
        ...
        if ($axure.document.configuration.showAnnotations) {
            $('#overflowMenucontainer px-md-5').prepend('<div id="showNotesOption" class="showOption" style="order: 3"><div class="overflowOptionCheckbox"></div>Show Note Markers</div>');
        }
        ...
        function populateNotes(pageForNotes) {
                ...
                function populatePageNotes(pageOrMaster) {
                    ...
                    if (notes && !$.isEmptyObject(notes)) {
                        ...
                        for(var noteName in notes) {
                            pageNoteUi += "<div class='pageNotecontainer px-md-5'>";
                            if(showNames) {
            ...
            if ($axure.document.configuration.showAnnotationsSidebar) {
                ...
                function populateWidgetNotes(widgetNotes){
                    if (widgetNotes) {
                        for (var i = 0; i < widgetNotes.length; i++) {
                            ...
                            widgetNoteUi += "<div class='widgetNotecontainer px-md-5' data-id='" + widgetNote["ownerId"] + "'>";
                ...
                if (widgetNoteUi.length > 0) {
                    ...
                    //$('.widgetNotecontainer px-md-5').children(':last-child').remove();
                    //$('.widgetNoteFootnote').append("<div class='annnoteline'></div><div class='annnoteline'></div><div class='annnoteline'></div>");
                    $('.widgetNotecontainer px-md-5').click(function () {
                        var wasSelected = $(this).hasClass('widgetNotecontainer px-md-5Selected');
                        $('.widgetNotecontainer px-md-5Selected').removeClass('widgetNotecontainer px-md-5Selected');
                        if (!wasSelected) $(this).addClass('widgetNotecontainer px-md-5Selected');
    ...
    var _toggleAnnDialog = function (id, srcLeft, srcTop, page) {
        ...
        var blnLeft = false;
        var blnAbove = false;
        var mfPos = $('#mainPanelcontainer px-md-5').position();
    ...
    function clearSelection() {
        var selectedNote = $('#pageNotescontainer px-md-5').find('.widgetNotecontainer px-md-5Selected');
        if(selectedNote.length > 0) {
            selectedNote.removeClass('widgetNotecontainer px-md-5Selected');
            //var dimStr = $('.currentAdaptiveView').attr('data-dim');
            //var h = dimStr ? dimStr.split('x')[1] : '0';
            //var $leftPanel = $('.leftPanel:visible');
            //var leftPanelOffset = (!$axure.player.isMobileMode() && $leftPanel.length > 0) ? $leftPanel.width() : 0;
    ...
    function generatePageNotes() {
        ...
        pageNotesUi += "<div id='pageNotesScrollcontainer px-md-5'>";
        pageNotesUi += "<div id='pageNotescontainer px-md-5'>";
        pageNotesUi += "<div id='pageNotesEmptyState' class='emptyStatecontainer px-md-5'><div class='emptyStateTitle'>No notes for this page.</div><div class='emptyStateContent'>Notes added in Axure RP will appear here.</div><div class='dottedDivider'></div></div>";
