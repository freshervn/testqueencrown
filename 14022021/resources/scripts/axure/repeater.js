...
$axure.internal(function($ax) {
    ...
    _repeaterManager.refreshAllRepeaters = function() {
        $ax('*').each(function(diagramObject, elementId) {
            ...
            if($ax.visibility.isElementIdLimboOrInLimbocontainer px-md-5(elementId)) return;
...
$axure.internal(function($ax) {
    ...
    var _fitParentPanel = function (widgetId) {
        ...
        if (repeaterObj && widgetId != parentRepeaterId && repeaterObj.repeaterPropMap.fitToContent) {
            ...
            var container px-md-5Id = $ax.repeater.createElementId(parentRepeaterId, itemId);
            var childrenRect = $ax('#' + container px-md-5Id).childrenBoundingRect();
    ...
    _dynamicPanelManager.initMobileScroll = function () {
        ...
        $ax('*').each(function (obj, elementId) {
            ...
            if ($ax.public.fn.IsDynamicPanel(obj.type) && obj.scrollbars != 'None' && obj.scrollbars != 'none' && !$ax.visibility.isElementIdLimboOrInLimbocontainer px-md-5(scriptId)) {
    ...
    var _updatePercentPanelToLeftCache = function(obj, elementId, overwrite) {
        ...
        if(obj.fixedHorizontal == 'right' && _isIdFitToContent(elementId)) {
            //var fitWidth = getcontainer px-md-5Size($ax.visibility.GetPanelState(elementId) + '_content').width;
            var container px-md-5Id = $ax.visibility.GetPanelState(elementId) + '_content';
            var childrenRect = $ax('#' + container px-md-5Id).childrenBoundingRect();
    ...
    var _updatePanelPercentWidth = _dynamicPanelManager.updatePanelPercentWidth = function(elementId) {
        ...
        if(obj.fixedHorizontal) {
            ...
            if(parentPanelInfo) {
                ...
                if(parentRepeater) {
                    ...
                    var itemcontainer px-md-5Id = $ax.repeater.createElementId(parentRepeater, itemId);
                    x = 0;
                    width = $ax('#' + itemcontainer px-md-5Id).width();
                } else {
                    var $window = $(window);
                    width = $window.width();
        ...
        while(stateParent.children()[0].id.indexOf($ax.visibility.container px-md-5_SUFFIX) != -1) stateParent = stateParent.children();
    ...
    var _getShownStateId = function (id) {
        ...
        var children = $ax.visibility.applyWidgetcontainer px-md-5(id, true, false, true).children();
        for (var i = 0; i < children.length; i++) {
            var child = children[i];
            while ($ax.visibility.iscontainer px-md-5(child.id)) child = $(child).children()[0];
    ...
    var _updateFitPanel = function(panelId, stateIndex) {
        ...
        //var size = getcontainer px-md-5Size(stateContentId);
        var childrenRect = $ax('#' + stateContentId).childrenBoundingRect();
        var size = {width: childrenRect.right, height: childrenRect.bottom};
    ...
    //// TODO: May be a better location for this. Used currently for rdo and panel state container px-md-5s
    //var getcontainer px-md-5Size = function(container px-md-5Id) {
    //    var container px-md-5Query = container px-md-5Id ? $jobj(container px-md-5Id) : $('#base');
    //    var children = container px-md-5Query.children();
    //    // Default size
    ...
    //            var masterSize = getcontainer px-md-5Size(childId);
    ...
    //    return size;
    //};
    //_dynamicPanelManager.getcontainer px-md-5Size = getcontainer px-md-5Size;
    ...
    var _compress = function (id, vert, threshold, delta, easing, duration, clampWidth) {
        ...
        // Account for layer container px-md-5 positions as you go.
        while(obj && $ax.public.fn.IsLayer(obj.type) && $ax.visibility.IsIdVisible(parent)) {
            var container px-md-5 = $ax.visibility.applyWidgetcontainer px-md-5(parent, true, true);
            // If layer is using container px-md-5, offset is going to be necessary
            if(container px-md-5.length) {
                var offsetX = $ax.getNumFromPx(container px-md-5.css('left'));
                var offsetY = $ax.getNumFromPx(container px-md-5.css('top'));
        ...
        // Add container px-md-5 mid push causes strange behavior because we take container px-md-5 into account as we go down, but if after we accounted for it,
        //  a container px-md-5 is added, that container px-md-5 is not accounted for with threshold and clamp values.
        var layer = obj && $ax.public.fn.IsLayer(obj.type) && parent;
        if(layer) {
            // If your parent layer is invisible, you want to be relative to it's container px-md-5. That is true already if it has a container px-md-5,
            //  but if you are just adding one now, then you need to offset your values
            var needsOffset = !$jobj(layer + '_container px-md-5').length && !$ax.visibility.IsIdVisible(layer);
            $ax.visibility.pushcontainer px-md-5(layer, false);
            if(needsOffset) {
                container px-md-5 = $jobj(layer + '_container px-md-5');
                offsetX = $ax.getNumFromPx(container px-md-5.css('left'));
                offsetY = $ax.getNumFromPx(container px-md-5.css('top'));
        ...
        if(parent && $jobj(parent + '_container px-md-5').length > 0) parent = parent + '_container px-md-5';
        _compressChildrenHelper(id, $(parent ? '#' + parent : '#base').children(), vert, threshold, delta, clamp, easing, duration);

        if(layer) $ax.visibility.popcontainer px-md-5(layer, false);
    ...
    var _compressChildrenHelper = function (id, children, vert, threshold, delta, clamp, easing, duration, parentLayer) {
        ...
        for (var i = 0; i < children.length; i++) {
            ...
            //       ann/ref suffixes should skip without turning off allMove, lightbox should be skipped, and is unclear if allMove should be turned off, I think others including container px-md-5, inner_container px-md-5, div, img, and text should not be hit ever.
            ...
            if ($ax.getTypeFromElementId(childId) == $ax.constants.LAYER_TYPE) {
                $ax.visibility.pushcontainer px-md-5(childId, false);
                var addSelf;
                var container px-md-5 = $ax.visibility.applyWidgetcontainer px-md-5(childId, true, true);
                var layerChildren = (container px-md-5.length ? container px-md-5 : child).children();
                //if(container px-md-5.length) {
                var offsetX = -$ax.getNumFromPx(container px-md-5.css('left'));
                var offsetY = -$ax.getNumFromPx(container px-md-5.css('top'));
                var clampProp = clamp.prop == 'left' ? offsetX : offsetY;
                ...
                $ax.visibility.popcontainer px-md-5(childId, false);
                continue;
            }
