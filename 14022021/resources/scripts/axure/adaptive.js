$axure.internal(function($ax) {
    ...
    var _removeNiceScroll = $ax.adaptive.removeNiceScroll = function ($container px-md-5, blockResetScroll) {
        if (!blockResetScroll) {
            $container px-md-5.scrollLeft(0);
            $container px-md-5.scrollTop(0);
        }
        $container px-md-5.getNiceScroll().remove();
        //clean up nicescroll css
        if (IE) $container px-md-5.css({ '-ms-overflow-y': '', 'overflow-y': '', '-ms-overflow-style': '', '-ms-touch-action': '' });
    }

    var _addNiceScroll = $ax.adaptive.addNiceScroll = function ($container px-md-5, options, blockResetScroll) {
        if (!blockResetScroll) {
            $container px-md-5.scrollLeft(0);
            $container px-md-5.scrollTop(0);
        }
        $container px-md-5.niceScroll(options);
        //clean up nicescroll css so child scroll container px-md-5s show scrollbars in IE
        if (IE) $container px-md-5.css({ '-ms-overflow-y': '', '-ms-overflow-style': '' });
        if(IOS) $container px-md-5.css({ 'overflow-y': ''});
    }
