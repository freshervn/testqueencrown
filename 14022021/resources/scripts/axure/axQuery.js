   ...
   $axure.resize = function(fn) { 
    ...
    $ax.public.fn.getParents = function (deep, filter) {
        ...
        var getParent = function(elementId) {
            var container px-md-5Index = elementId.indexOf('_container px-md-5');
            if(container px-md-5Index !== -1) elementId = elementId.substring(0, container px-md-5Index);
        ...
        for(var i = 0; i < elementIds.length; i++) {
            ...
            if(deep) {
                ...
                while(parent) {
                    ...
                    //if(!$obj(parent)) parent = $ax.visibility.getWidgetFromcontainer px-md-5($jobj(parent).parent().attr('id'));
        ...
        var getChildren = function (elementId) {
            ...
            //if (isRepeater || isDynamicPanel || isLayer || isMaster || isMenu || isTreeNode || isTable) {// || isCompoundVector) {
                ...
                if(isRepeater) {
                    ...
                    // Get through all container px-md-5s
                    while ($(parent[0]).attr('id').indexOf('container px-md-5') != -1) parent = parent.children();
