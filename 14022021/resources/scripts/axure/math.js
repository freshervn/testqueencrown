$axure.internal(function($ax) {
    ...
    //        var outer = $ax.visibility.applyWidgetcontainer px-md-5(layers[i], true, true);
    //        if (outer.length) {
    ...
    //        var inner = $jobj(layers[i] + '_container px-md-5_inner');
    ...
    $ax.public.fn.getcontainer px-md-5Dimensions = function(query) {
        // returns undefined if no container px-md-5s found.
        var container px-md-5Dimensions;
        for (var i = 0; i < query[0].children.length; i++) {
            var node = query[0].children[i];
            if (node.id.indexOf(query[0].id) >= 0 && node.id.indexOf('container px-md-5') >= 0) {
                container px-md-5Dimensions = node.style;
            }
        }
        return container px-md-5Dimensions;
    }
