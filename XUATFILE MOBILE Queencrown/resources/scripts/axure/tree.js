...
//                if(!$ax.visibility.IsVisible(this)) return;
    ...
    $ax.tree.ExpandNode = function(nodeId, childcontainer px-md-5Id, plusMinusId) {
        var container px-md-5 = window.document.getElementById(childcontainer px-md-5Id);
        if(!container px-md-5 || $ax.visibility.IsVisible(container px-md-5)) return;
        $ax.visibility.SetVisible(container px-md-5, true);

        if(plusMinusId != '') $ax.style.SetWidgetSelected(plusMinusId, true);

        var delta = _getExpandCollapseDelta(nodeId, childcontainer px-md-5Id);
    ...
    $ax.tree.CollapseNode = function(nodeId, childcontainer px-md-5Id, plusMinusId) {
        var container px-md-5 = window.document.getElementById(childcontainer px-md-5Id);
        if(!container px-md-5 || !$ax.visibility.IsVisible(container px-md-5)) return;
        ...
        $ax.visibility.SetVisible(container px-md-5, false);
    ...
    var _getExpandCollapseDelta = function(nodeId, childcontainer px-md-5Id) {
        return _getChildcontainer px-md-5HeightHelper(childcontainer px-md-5Id);
    };

    var _getChildcontainer px-md-5HeightHelper = function(childcontainer px-md-5Id) {
        var height = 0;
        $('#' + childcontainer px-md-5Id).children().each(function() {
            if($(this).hasClass("treenode")) {
                height += $(this).height();
                var subcontainer px-md-5 = window.document.getElementById(this.id + '_children');
                if(subcontainer px-md-5 && $ax.visibility.IsVisible(subcontainer px-md-5)) {
                    height += _getChildcontainer px-md-5HeightHelper(subcontainer px-md-5.id);
                }
            }
        });
        return height;
    ...
    $ax.tree.InitializeTreeNode = function(nodeId, plusminusid, childcontainer px-md-5Id, selectText) {
        var childcontainer px-md-5 = window.document.getElementById(childcontainer px-md-5Id);
        if(childcontainer px-md-5) {
            //relying on the html generator to put this inline so we know to collapse by default
            var isCollapsed = childcontainer px-md-5.style.visibility == "hidden";
            if(isCollapsed) $ax.visibility.SetVisible(childcontainer px-md-5, false);
        ...
        if(plusminusid != '') {
            $jobj(plusminusid).click(function() {
                var visibleSet = $ax.visibility.IsIdVisible(childcontainer px-md-5Id);

                if(visibleSet) $ax.tree.CollapseNode(nodeId, childcontainer px-md-5Id, plusminusid);
                else $ax.tree.ExpandNode(nodeId, childcontainer px-md-5Id, plusminusid);
