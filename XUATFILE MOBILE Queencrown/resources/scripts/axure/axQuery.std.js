...
$axure.internal(function($ax) {
    ...
    $ax.public.fn.Iscontainer px-md-5 = function (type) { return type== $ax.constants.VECTOR_SHAPE_TYPE || type == $ax.constants.TABLE_TYPE || type == $ax.constants.MENU_OBJECT_TYPE || type == $ax.constants.TREE_NODE_OBJECT_TYPE; }
...
//            var wasShown = $ax.visibility.IsIdVisible(elementId);
    ...
    var _move = function (elementId, x, y, options, moveTo) {
        ...
        // Layer move using container px-md-5 now.
    ...
    $ax.public.fn.resize = function(newLocationAndSizeCss, resizeInfo, axShouldFire, moves, onCompletedFunc) {
        ...
        for(var index = 0; index < elementIds.length; index++) {
            ...
            while(children && children.length && $(children[0]).attr('id').indexOf('container px-md-5') != -1) {
        ...
        if(arguments[0] == undefined) {
            ...
            for(var index = 0; index < elementIds.length; index++) {
                if ($ax.public.fn.IsTreeNodeObject($ax.getTypeFromElementId(elementIds[index]))) {
                    ...
                    var childcontainer px-md-5Id = treeNodeId + '_children';
                    ...
                    if($('#' + childcontainer px-md-5Id).length == 0 || !$jobj(plusMinusId).children().first().is('img'))
                        plusMinusId = '';

                    if(arguments[0] == true) {
                        $ax.tree.ExpandNode(treeNodeId, childcontainer px-md-5Id, plusMinusId);
                    } else if(arguments[0] == false) {
                        $ax.tree.CollapseNode(treeNodeId, childcontainer px-md-5Id, plusMinusId);
                    }
                }
            }
    ...
    $ax.public.fn.getPageSize = function() {
        var container px-md-5Query = $('#base');
    ...
    $ax.public.fn.pageBoundingRect = function (ignoreRotation, scrollableId) {
        ...
        //while ($ax.public.fn.Iscontainer px-md-5(parObj.type)) {
        ...
        for (var i = 0; i < parentIds.length; i++) {
            //var parentId = $ax.visibility.getWidgetFromcontainer px-md-5(parents[0]);
            //var parent = $ax.visibility.applyWidgetcontainer px-md-5(parentId, true);
            //if(parent.length) {
    ...
    //        for(var i = 0; i < parents.length; i++) {
    //            var parentId = $ax.visibility.getWidgetFromcontainer px-md-5(parents[i]);
    //            var parent = $ax.visibility.applyWidgetcontainer px-md-5(parentId, true);

    //            // Layer may not have container px-md-5, and will be at 0,0 otherwise.
    ...
    //    // Special Layer code
    //    if (axObj.type == 'layer') {
    //        // If layer has a container px-md-5, then use that. Otherwise must deal with children. Children can move in container px-md-5 after created, but ignoring for now.
    //        var container px-md-5 = $ax.visibility.applyWidgetcontainer px-md-5(id, true, true);
    //        if(container px-md-5.length) loc += $ax.getNumFromPx(container px-md-5.css(prop));
