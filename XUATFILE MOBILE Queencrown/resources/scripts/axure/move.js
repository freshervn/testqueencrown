$axure.internal(function($ax) {
    ...
    var widgetMoveInfo = {};
    //register and return move info, also create container px-md-5 for rootlayer if needed
    $ax.move.PrepareForMove = function (id, x, y, to, options, jobj, rootLayer, skipcontainer px-md-5ForRootLayer) {
        ...
        if(!rootLayer) {
            ...
            if (rootLayer && !skipcontainer px-md-5ForRootLayer) {
                $ax.visibility.pushcontainer px-md-5(rootLayer, false);
                if (isLayer) widget = $ax.visibility.applyWidgetcontainer px-md-5(id, true);
            }
        }
        if (!jobj) jobj = widget;
    ...
    $ax.move.MoveWidget = function (id, x, y, options, to, animationCompleteCallback, shouldFire, jobj, skipOnMoveEvent) {
        ...
        if(object && $ax.public.fn.IsLayer(object.type)) {
            ...
            //don't push container px-md-5 when register moveinfo for child
            if(!skipOnMoveEvent) {
    ...
    var _moveElement = function (id, options, animationCompleteCallback, shouldFire,  jobj, moveInfo){
        ...
        // I don't think root layer is necessary anymore after changes to layer container px-md-5 structure.
        ...
        var completeAnimation = function() {
            completeCount--;
            if(completeCount == 0 && rootLayer) $ax.visibility.popcontainer px-md-5(rootLayer, false);
        ...
        if(options.easing==='none') {
            ...
            if(rootLayer) $ax.visibility.popcontainer px-md-5(rootLayer, false);
            if(animationCompleteCallback) animationCompleteCallback();
    ...
    _move.circularMove = function (id, degreeDelta, centerPoint, moveDelta, rotatableMove, resizeOffset, options, fireAnimationQueue, completionCallback, willDoRotation) {
        ...
        var onComplete = function() {
            ...
            if(completionCallback) completionCallback();
            if(moveInfo.rootLayer) $ax.visibility.popcontainer px-md-5(moveInfo.rootLayer, false);
