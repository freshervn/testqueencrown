...
(function () {
    ...
    function appendOutOfDateNotification() {
        ...
        toAppend += '   <div id="supportedBrowsersListcontainer px-md-5">';
        toAppend += '       <div class="browsercontainer px-md-5">';
    ...
    function initializeEvents() {
        $('#interfaceControlFrameMinimizecontainer px-md-5').on('click', collapse);
        $('#interfaceControlFrameCloseButton').on('click', closePlayer);
        $('#interfacePageNamecontainer px-md-5').on($axure.eventNames.mouseDownName, toggleSitemap);
        $('#interfaceAdaptiveViewscontainer px-md-5').on($axure.eventNames.mouseDownName, toggleAdaptiveViewsPopup);
        ...
        $minimizecontainer px-md-5 = $('#interfaceControlFrameMinimizecontainer px-md-5');
        $minimizecontainer px-md-5.mouseenter(function () { $minimizecontainer px-md-5.addClass('collapseHovered') });
        $minimizecontainer px-md-5.mouseleave(function () { $minimizecontainer px-md-5.removeClass('collapseHovered') });
        $maximizecontainer px-md-5 = $('#maximizePanelcontainer px-md-5');
        $maximizecontainer px-md-5.mouseenter(function () { if(!MOBILE_DEVICE) $minimizecontainer px-md-5.addClass('expandHovered') });
        $maximizecontainer px-md-5.mouseleave(function () { if(!MOBILE_DEVICE) $minimizecontainer px-md-5.removeClass('expandHovered') });

        $('#maximizePanel').click(function () {
            $(this).removeClass('maximizePanelOver');
            $('#maximizePanelcontainer px-md-5').hide();
    ...
    function initializeDeviceFrame() {
        ...
        // - Determine sizing, positioning, orientation, and styling for HTML container px-md-5s
        ...
        if (device === 12) {
            // Hide container px-md-5s
            $('#deviceFramecontainer px-md-5').hide();
            $('#bezelOverlay').hide();
    ...
    function toggleSitemapMobileMode() {
        var $container px-md-5 = $('#sitemapHost');
        if (!$container px-md-5.length) return;
        var $header = $container px-md-5.find('.pluginNameHeader');
        var projectName = $axure.player.getProjectName();

        if (isMobileMode()) {
            $header.text('PROJECT PAGES');
            $container px-md-5.addClass('mobileMode');
            $container px-md-5.find('.sitemapPageName').addClass('mobileText');
            // Give sitemapHost left-margin so it does not collide with projectOptionsHost
            if (MOBILE_DEVICE) $container px-md-5.css('margin-left', '13px');
        } else {
            $container px-md-5.removeClass('mobileMode');
            $header.text(projectName ? projectName : 'Pages');
            $container px-md-5.find('.sitemapPageName').removeClass('mobileText');
            if (MOBILE_DEVICE) $container px-md-5.css('margin-left', '');
        }
    }

    function togglePageNotesMobileMode() {
        var $container px-md-5 = $('#pageNotesHost');
        if (!$container px-md-5.length) return;
        ...
        if (isMobileMode()) {
            ...
            $container px-md-5.find('.notesPageNameHeader').addClass('mobileSubHeader');
            $container px-md-5.find('.pageNote').addClass('mobileText');
            $container px-md-5.find('.emptyStateTitle').addClass('mobileSubHeader');
            $container px-md-5.find('.emptyStateContent').addClass('mobileText');
        } else {
            $container px-md-5.removeClass('mobileMode');
            $('#pageNotesSectionHeader').text('Page Notes');
            $('#widgetNotesSectionHeader').text('Widget Notes');
            $container px-md-5.find('.notesPageNameHeader').removeClass('mobileSubHeader');
            $container px-md-5.find('.pageNote').removeClass('mobileText');
            $container px-md-5.find('.emptyStateTitle').removeClass('mobileSubHeader');
            $container px-md-5.find('.emptyStateContent').removeClass('mobileText');
        }
    
    }

    function toggleFeedbackMobileMode() {
        var $container px-md-5 = $('#feedbackHost');
        if (!$container px-md-5.length) return;
    ...
    function deactivateMobileTextEntry() {
        ...
        $('#mobileControlFramecontainer px-md-5').show();
    }

    function activateMobileTextEntry() {
        $('#mobileControlFramecontainer px-md-5').hide();
    ...
    $axure.player.resizeContent = function (noViewport) {
        ...
        if (wasMobile && !isMobile) {
            $('#clippingBoundsScrollcontainer px-md-5').show();
            $('#outercontainer px-md-5').prepend($('.leftPanel'));
            $('#outercontainer px-md-5').append($('.rightPanel'));
            ...
            if ($('#topPanel').is(':visible')) {
                ...
                $('.leftPanel').hide();
                $('.rightPanel').hide();
                if (!MOBILE_DEVICE) $('#maximizePanelcontainer px-md-5').show();
            }
            ...
            $('#clippingBoundsScrollcontainer px-md-5').hide();
            ...
            if (MOBILE_DEVICE) $('#maximizePanelcontainer px-md-5').hide();
        ...
        $('#outercontainer px-md-5').height(newHeight).width(newWidth);
        $('#mainPanel').height(newHeight);
        $('#clippingBounds').height(newHeight);

        if (isMobile) {
            $('#mobileControlFramecontainer px-md-5').height(newHeight);
            $('#mobileControlFramecontainer px-md-5').width(newWidth);
            ...
            if ($('#mHideSidebar').is(':visible') && !$('#mobileControlFramecontainer px-md-5').is(':visible')) {
                $('#mobileControlFramecontainer px-md-5').delay(150).show();
    ...
    function calculateClippingBoundsScrollPosition() {
        // Adjust for mainPanelcontainer px-md-5 scaling (scale should be "none" for scaleVal == 0 or scaleVal == 1)
        var $iframe = $($('#mainPanel').find('iframe')[0].contentWindow);
        ...
        var scale = $('#mainPanelcontainer px-md-5').css('transform');;
        ...
        var viewablePanelLeftMargin = parseInt($('#mainPanelcontainer px-md-5').css('margin-left'));
        var viewablePanelTop = parseInt($('#mainPanelcontainer px-md-5').css('top'));
        if (isNaN(viewablePanelTop)) viewablePanelTop = 0;
        if (scaleVal == 2) {
            // Scale to Fit (account for main panel container px-md-5 scale) -- needed for device mode in Scale to Fit
            viewablePanelLeftMargin = ($('#mainPanel').width() - ($('#mainPanelcontainer px-md-5').width() * scale)) / 2
            viewablePanelTop = ($('#mainPanel').height() - ($('#mainPanelcontainer px-md-5').height() * scale)) / 2
        }
        ...
        else if (isCentered && scaleVal == 2) leftPos = $('#mainPanelcontainer px-md-5').width() * scale / 2.0 - contentLeftOfOriginOffset;

        // Include clipFrameScroll offset in mainPanelcontainer px-md-5
    ...
    function repositionClippingBoundsScroll() {
        ...
        (function repositionWhenReady() {
            if ($($('#mainPanel').find('iframe')[0].contentWindow.document.body).length === 0) {
                ...
                $('#clippingBoundsScrollcontainer px-md-5').css('left', position.left + 'px');
                $('#clippingBoundsScrollcontainer px-md-5').css('top', position.top + 'px');
            }
        })();
    }
    ...
    function calculateScrollLeftWithOffset(offset, isLeftPanel) {
        ...
        if (hasFrame) {
            var viewablePanelLeftMargin = parseInt($('#mainPanelcontainer px-md-5').css('margin-left'));
            var viewablePanelRightMargin = parseInt($('#mainPanelcontainer px-md-5').css('margin-right'));
    ...
    $axure.player.collapseToBar = function (context, hostId) {
        ...
        if (context === 'project' || context === 'all') {
            ...
            if(!isMobileMode()) {
                ...
                $('#clippingBoundsScrollcontainer px-md-5').animate({ left: newLeft + 'px' },
            ...
            if(!isMobileMode()) {
                ...
                $('#clippingBoundsScrollcontainer px-md-5').animate({ left: newLeft + 'px' },
                    { duration: 200, complete: function () {
                        isAnimating = false;
                        $axure.player.resizeContent();
                        if (isFinalPluginToRestore) $('#clippingBoundsScrollcontainer px-md-5').show();
    ...
    $axure.player.refreshViewPort = function () {
        ...
        $('#mainPanelcontainer px-md-5').attr({
        ...
        if (clipToView) {
            ...
            $('#mainPanelcontainer px-md-5').css({
                'margin': 'auto',
                'top': y + 'px'
            });
            ...
            $('#mainPanelcontainer px-md-5').width(w);
            $('#mainPanelcontainer px-md-5').height(h);
        } else {
            $('#mainFrame').width('100%');
            $('#mainFrame').height(h);
            ...
            $('#mainPanelcontainer px-md-5').width('100%');
            $('#mainPanelcontainer px-md-5').height(h);
            $('#mainPanelcontainer px-md-5').css({
                'left': '',
                'margin': '',
        ...
        $(".vpScaleOption").show();
        var prevScaleN = $('#mainPanelcontainer px-md-5').css('transform');
    ...
    function initializeLogo() {
        ...
        //if (typeof PREVIEW_INFO !== 'undefined') {
        //    $('#interfaceControlFrameLogoCaptioncontainer px-md-5').html(PREVIEW_INFO.fileName);
        //} else if (typeof $axure.player.settings.projectName !== 'undefined') {
        //    $('#interfaceControlFrameLogoCaptioncontainer px-md-5').html($axure.player.settings.projectName);
        //} else {
        //    $('#interfaceControlFrameLogoCaptioncontainer px-md-5').hide();
        //}
        ...
        //    //    $('#interfaceControlFramecontainer px-md-5').css('margin-left', '-' + $('#logoImage').width() / 2 + 'px');
        //    //    //$axure.player.resizeContent();
        //    //};
        //    image.src = $axure.document.configuration.logoImagePath;

        //    $('#interfaceControlFrameLogoImagecontainer px-md-5').html('<img id="logoImage" src="" />');
        //    $('#logoImage').attr('src', $axure.document.configuration.logoImagePath);//.on('load', function () { $axure.player.resizeContent(); });
        //} else $('#interfaceControlFrameLogoImagecontainer px-md-5').hide();

        //if ($axure.document.configuration.logoImageCaption) {
        //    $('#interfaceControlFrameLogoCaptioncontainer px-md-5').html($axure.document.configuration.logoImageCaption);
        //} else $('#interfaceControlFrameLogoCaptioncontainer px-md-5').hide();

        //if(!$('#interfaceControlFrameLogoImagecontainer px-md-5').is(':visible') && !$('#interfaceControlFrameLogoCaptioncontainer px-md-5').is(':visible')) {
        //    $('#interfaceControlFrameLogocontainer px-md-5').hide();
        //}
    }
    
    function initializePreview() {
        if (typeof PREVIEW_INFO !== 'undefined') {
            $('#separatorcontainer px-md-5').addClass('hasLeft');
    ...
    */
    function buildAccountLoginPopup() {
        return [
            ...
            '   <div class="axureLoginBubblecontainer px-md-5_Player">',
    ...
    function bindAxureLogincontainer px-md-5Event() {
        var $container px-md-5 = $("#accountLogincontainer px-md-5");
        $container px-md-5.find('input[name="email"]').addClass("watermark").val(emailHint).focus(function () {
            ...
            $container px-md-5.find('.errorMessage').text('');
            $container px-md-5.find('.errorMessage').hide();
        }).keyup(function (event) {
            if (event.keyCode == 13) {
                $container px-md-5.find('.feedbackGreenBtn').click();
            }
        });
        $container px-md-5.find('input[name="password"]').addClass("watermark").val(passHint).focus(function () {
            ...
            if (event.keyCode == 13) {
                $container px-md-5.find('.feedbackGreenBtn_Player').click();
            }
        });

        // Login Submit Event
        $container px-md-5.find('.feedbackGreenBtn_Player').click(function (e) {
            var email = $container px-md-5.find('.axureEmail').val();
            var password = $container px-md-5.find('.axurePassword').val();

            $axure.player.axureLogin(email, password, function (response) {
                // Success
                // Clear out fields
                $container px-md-5.find('.axureEmail').val(emailHint).addClass("watermark");
                $container px-md-5.find('.axurePassword').val(passHint).addClass("watermark");
                $container px-md-5.find('.axurePassword')[0].setAttribute('type', 'text');

                closePopup();
            }, function (response) {
                // Failure
                $container px-md-5.find('.errorMessage').text(response != null && response.message ? response.message : "There was an error connecting to the server, please try again later.");
                $container px-md-5.find('.errorMessage').show();
            }, function (response) {
                // SAML User
                $container px-md-5.find('.errorMessage').empty();
                $container px-md-5.find('.errorMessage').append("Please <a class='refreshLink' style='text-decoration: underline;'>refresh</a> this page after logging in via your identity provider.");
                $container px-md-5.find('.errorMessage').show();

                window.open(response.redirecturl, '_blank');

                $container px-md-5.find('.errorMessage').find('.refreshLink').click(function () {
                ...
                if (window.$axshare == null || $axshare.auth == null || $axshare.login == null) {
                    ...
                    $.ajax({
                        ...
                        success: function (response) {
                            ...
                            if (isSubInstance) $('#accountLogincontainer px-md-5').find("#playerSignUpLink").hide();
                    ...
                    // Login container px-md-5
                    $("#accountLogincontainer px-md-5").append(buildAccountLoginPopup());
                    bindAxureLogincontainer px-md-5Event();
    ...
    function appendProjectOptions() {
        ...
        toAppend += '    <div id="projectOptionsScrollcontainer px-md-5">';
        ...
        toAppend += '       <div id="projectOptionsScalecontainer px-md-5" class="mobileText"></div>';
        toAppend += '       <div id="projectOptionsAdaptiveViewsHeader" class="mobileSubHeader" style="margin-top: 16px">Adaptive Views</div>';
        toAppend += '       <div id="projectOptionsAdaptiveViewscontainer px-md-5" class="mobileText"></div>'
        toAppend += '    </div>'
        toAppend += '</div>';
    ...
    function appendMobileBrowserControlFrame() {
        ...
        $('#mobileControlFramecontainer px-md-5').append(toAppend);
        ...
        if (IOS) $('#mobileControlFramecontainer px-md-5').on($axure.eventNames.mouseDownName, function (e) { e.stopPropagation(); });
    }
    ...
    function closePopup() {
        var $container px-md-5 = $('.popup');
        var isLeftPanel = $container px-md-5.hasClass('leftPanel');
        $container px-md-5.removeClass('popup');
        $('#overflowMenuButton').removeClass('selected');
        $('#interfaceAdaptiveViewscontainer px-md-5').removeClass('selected');
        $container px-md-5.hide();
    ...
    function showPopup($container px-md-5) {
        if ($('#browserOutOfDateNotification').is(":visible")) return;
        $container px-md-5.addClass('popup');
        $container px-md-5.show();

        $('<div class="splitterMask"></div>').insertAfter($container px-md-5);
    ...
    function toggleAdaptiveViewsPopup() {
        if (($('#interfaceAdaptiveViewsListcontainer px-md-5').hasClass('popup'))) {
            closePopup();
        } else {
            $('#interfaceAdaptiveViewscontainer px-md-5').addClass('selected');
            showPopup($('#interfaceAdaptiveViewsListcontainer px-md-5'));
        }
    }

    function toggleOverflowMenuPopup() {
        if (($('#overflowMenucontainer px-md-5').hasClass('popup'))) {
            closePopup();
        } else {
            $('#overflowMenuButton').addClass('selected');
            showPopup($('#overflowMenucontainer px-md-5'));
        }
    }
    ...
    function startM(e) {
        ...
        // (likely due to position of click being calculated as out of bounds for outercontainer px-md-5 -- iOS is not adding scroll offset to bounds)
    ...
    function repositionPinsOnScaleChange(data) {
        ...
        for (var i = 0; i < $pins.length; i++) {
            ...
            // Get current scale of mainPanelcontainer px-md-5
            // MainPanelcontainer px-md-5 scaled without setContentScale message
            var scale = $('#mainPanelcontainer px-md-5').css('transform');;
            scale = (scale == "none") ? 1 : Number(scale.substring(scale.indexOf('(') + 1, scale.indexOf(',')));
        ...
        if (message == 'expandFrame') expand();
            ...
            if (data.clipToView) {
                ...
                if (data.scaleN != 1) {
                    ...
                    var height = Number($('#mainPanelcontainer px-md-5').css('height').replace('px', '')) / data.scaleN + 'px';
                    $('#mainPanelcontainer px-md-5').css({
                        'transform': scale,
    ...
    function expand() {
        if ($axure.player.isMobileMode()) {
            ...
            $minimizecontainer px-md-5 = $('#interfaceControlFrameMinimizecontainer px-md-5');
            $minimizecontainer px-md-5.removeClass('collapseHovered');
    ...
    function setUpController() {
        ...
        _player.createPluginHost = function (settings) {
            ...
            if (settings.id == 'feedbackHost')
                $('#overflowMenucontainer px-md-5').prepend('<div id="showCommentsOption" class="showOption" style="order: 2"><div class="overflowOptionCheckbox"></div>Show Comments</div>');
            ...
            if (typeof PREVIEW_INFO === 'undefined') {
                ...
                if (settings.id == 'handoffHost') { $('#handoffControlFrameHeadercontainer px-md-5').show(); }
            } else {
                // Preview Mode
            ...
            var hostcontainer px-md-5Id = settings.context + 'ControlFrameHostcontainer px-md-5';
            hostcontainer px-md-5Id = _player.isMobileMode() ? 'mHideSidebar' : 'outercontainer px-md-5';
            var panelClass = 'rightPanel';
            var host;
            if (settings.context == 'project') {
                panelClass = 'leftPanel';
                if (_player.isMobileMode() && $('#' + hostcontainer px-md-5Id).find('#projectOptionsHost').length > 0) {
                    host = $('<div id="' + settings.id + '" class="' + panelClass + '"></div>')
                        .insertAfter('#projectOptionsHost');
                } else {
                    host = $('<div id="' + settings.id + '" class="' + panelClass + '"></div>')
                        .prependTo('#' + hostcontainer px-md-5Id);
                }
            } else {
                if (!$('#separatorcontainer px-md-5').hasClass('hasLeft')) $('#separatorcontainer px-md-5').addClass('hasLeft');
                host = $('<div id="' + settings.id + '" class="' + panelClass + '"></div>')
                    .appendTo('#' + hostcontainer px-md-5Id);
            }

            $(('#' + settings.id)).click(function (e) { e.stopPropagation(); });

            var controlcontainer px-md-5Id = getControlcontainer px-md-5Id(settings.id);
            ...
            headerLink.parent().appendTo('#' + controlcontainer px-md-5Id);
        ...
        var getControlcontainer px-md-5Id = function (id) {
        ...
        var interfaceControlHeaderButton_click = function (id) {
            ...
            var controlcontainer px-md-5Id = getControlcontainer px-md-5Id(id);
            var context = plugins[id].context;

            var clickedPlugin = $('#' + controlcontainer px-md-5Id + ' a[pluginId=' + id + ']');
            if (currentVisibleHostId[context] == id) {
                clickedPlugin.removeClass('selected');
                if (id == "sitemapHost") { $('#sitemapControlFramecontainer px-md-5').removeClass('selected'); }
                currentVisibleHostId[context] = -1;
                _player.collapseToBar(context, id);
                
                $(document).trigger('pluginShown', [getVisiblePlugins()]);
            } else {
                $('#' + controlcontainer px-md-5Id + ' a').removeClass('selected');
                clickedPlugin.addClass('selected');
                if (id == "sitemapHost") { $('#sitemapControlFramecontainer px-md-5').addClass('selected'); }
        ...
        _player.restorePlugins = function () {
            ...
            for (var id in plugins) {
                var clickedPlugin = $('#' + getControlcontainer px-md-5Id(id) + ' a[pluginId=' + id + ']');
                if (clickedPlugin.hasClass('selected')) selectedPluginsCount++;
            }
            if ($axure.player.settings.isAxshare && selectedPluginsCount != 0) $('#clippingBoundsScrollcontainer px-md-5').hide();
    ...
    $axure.player.hideAllPlayerControllers = function(isVisible) {
        // TOOD: Verify that the container px-md-5s are set to the right state after re-enabling them
        if(isVisible) {
            $('#topPanel').css('display', '');
            $('#popupcontainer px-md-5').css('display', '');
            $('#maximizePanelcontainer px-md-5').css('display', '');        
            $('#mobileControlFramecontainer px-md-5').css('display', '');
        } else {
            $('#topPanel').hide();
            $('#popupcontainer px-md-5').hide();
            $('#maximizePanelcontainer px-md-5').hide();        // TODO: This needs to have a function where it prevents itself from showing up externally
            $('#mobileControlFramecontainer px-md-5').hide();
        }
    }
    ...
    $axure.player.addDeviceFraming = function (project, isEdit) {
        ...
        // TODO: Generate html for overlay and bezel container px-md-5s
        // TODO: Determine if preview player or full prototype player to establish where container px-md-5s will be stored
        ...
        $overlayParent = isEdit && $overlayParent.length !== 0 ? $overlayParent : $('#mainPanelcontainer px-md-5');
        ...
        if (framePath != undefined) {
            $overlayParent.prepend(genFramecontainer px-md-5());

            var $fcontainer px-md-5 = $overlayParent.find('#deviceFramecontainer px-md-5');
            var $frame = $fcontainer px-md-5.find('#deviceFrame');
            ...
            if(isEdit) {
                $fcontainer px-md-5.css('z-index', -1);
            }
        }

        if (overlayPath != undefined) {
            // TODO: Update for edit mode
            // $overlayParent.append(genOverlaycontainer px-md-5());

            var $ocontainer px-md-5 = $overlayParent.find('#deviceOverlaycontainer px-md-5');
            var $overlay = $ocontainer px-md-5.find('#deviceOverlay');

            $overlay.css('background-image', "url('" + overlayPath + "')");
        }
    }

    function genFramecontainer px-md-5(bezelPath) {
        var container px-md-5 = [
            '<div id="deviceFramecontainer px-md-5">',
            '   <div id="deviceFrame">',
            '   </div>',
            '</div>'
        ].join("");
